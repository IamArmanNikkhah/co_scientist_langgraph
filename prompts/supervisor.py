...
def build_supervisor_prompt(
    ...
    return (
        ...
        "4.  **Determine if Literature Retrieval is the Next Task:** Assess if a literature search is strategically valuable. The `next_task` MUST be `literature` if any of the following conditions are met, BUT ONLY if no literature retrieval has occurred in the last 2 iterations (checked via `last_literature_iteration` in statistics):\n"
        "      * **Bootstrap Phase (`iteration_count < 2`):** Trigger a broad search based on the core `research_goal`.\n"
        "      * **Stagnation Detected (`iterations_since_improvement > 4` AND (`last_literature_iteration` is None OR `iteration_count - last_literature_iteration > 2`)):** Trigger a targeted search to find novel approaches, combining the `research_goal` with terms that differ from top hypotheses.\n"
        "      * **Idea Pool Exhaustion (`evolution_candidates < 2` AND `unreviewed_hypotheses == 0` AND (`last_literature_iteration` is None OR `iteration_count - last_literature_iteration > 2`)):** Trigger an exploratory search to find new concepts.\n"
        "      * **Periodic Knowledge Refresh (`iteration_count % 8 == 0` and `iteration_count > 0` AND (`last_literature_iteration` is None OR `iteration_count - last_literature_iteration > 1`)):** Trigger a routine search to incorporate the latest findings.\n"
        "5.  **Literature Cooldown Override:** If literature retrieval conditions are met but cooldown is active (literature occurred within last 2 iterations), force alternative actions in this priority:\n"
        "      * If `evolution_candidates > 0`, choose `evolve` with strategy based on current research phase.\n"
        "      * Otherwise, choose `generate` with appropriate mode and quantity to create new hypotheses.\n"
        "6.  If none of the above apply, score the remaining actions (`generate`, `evolve`, `terminate`) and select the best one based on the overall research state.\n"
        "7.  When the `next_task` is `generate`, determine the `generation_mode`:\n"
        ...
        "2.  **Key Statistics & Implications:** `iterations_since_improvement: 5` is the critical metric. This value is greater than 4, potentially triggering the \"Stagnation Detected\" rule for literature retrieval, but cooldown must be checked.\n"
        ...
        "      * **Literature Retrieval:** The \"Stagnation Detected\" condition (`iterations_since_improvement > 4`) is met (it's 5). Assuming `last_literature_iteration` is None or `iteration_count - last_literature_iteration > 2`, the cooldown allows literature retrieval. Therefore, the `next_task` MUST be `literature`. No further steps in the decision process need to be considered.\n"
        ...
        "```\n\n"
        "### Parameter Schemas:\n\n"
        "  * **`literature`**: `{{\"search_query\": \"<string>\", \"sources\": [\"pubmed\", \"crossref\", \"arxiv\", \"scholar\", \"perplexity\"], \"max_results\": <int>}}`\n"
        "    - Note: \"perplexity\" refers to web search via the Perplexity Sonar-Pro API.\n"
